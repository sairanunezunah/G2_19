<?php
    class Socios extends Conectar{
        ...
        public function get_sociosActivos(){
            $conectar = parent::conexion();
            parent::set_names();
            $sql = "SELECT * FROM ma_socios_negocio WHERE ESTADO = 'A'";
            $sql = $conectar->prepare($sql);
            $sql->execute();
            return $resultado=$sql->fetchAll(PDO::FETCH_ASSOC);
        }
        
        public function get_socioActivo($id){
            $conectar = parent::conexion();
            parent::set_names();
            $sql = "SELECT * FROM ma_socios_negocio WHERE ESTADO = 'A' AND ID=?";
            $sql = $conectar->prepare($sql);
            $sql->bindValue(1, $id);
            $sql->execute();
            return $resultado=$sql->fetchAll(PDO::FETCH_ASSOC);          
        ...
        public function insert_socio($nombre, $razon_social, $direccion, $tipo_socio, $contacto, $email, $fecha_creado, $estado, $telefono){
            ...
            $sql = $conectar->prepare("INSERT INTO ma_socios_negocio (NOMBRE, RAZON_SOCIAL, DIRECCION, TIPO_SOCIO, CONTACTO, EMAIL, FECHA_CREADO, ESTADO, TELEFONO) VALUES (:nombre, :razon_social, :direccion, :tipo_socio, :contacto, :email, :fecha_creado, :estado, :telefono)");
            ...
            $sql->bindParam(':tipo_socio', $tipo_socio);
            $sql->bindParam(':contacto', $contacto);
            $sql->bindParam(':email', $email);
            $sql->bindParam(':fecha_creado', $fecha_creado);
